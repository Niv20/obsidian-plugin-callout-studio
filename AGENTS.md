...
This file gives Codex, and other AI coding agents that support `AGENTS.md`, the minimum orientation needed to work in this repository --- it is a map, not the territory. For anything beyond the essentials below, follow the pointers to the real documentation; if it and this file ever disagree, the documentation wins.
...
- A handful of narrow subsystems have their own `.agents/skills/` entry instead of an internals-docs chapter (Tabler's outline icons, the "Your images" source, callout colour nesting, the metadata-pipe id split) --- see each skill's description for when it applies.
...
- **Do not expand or routinely update `AGENTS.md` when implementing changes.** Keep it short and stable. Update the relevant files in `docs/internals-docs/` instead, and let `AGENTS.md` point coding agents to that directory. If a change affects users, update `docs/user-guide/` as well. Only modify `AGENTS.md` when its navigation, essential project-level instructions, or documentation paths themselves become inaccurate.
...
Versions: bump `manifest.json`, `package.json`, `package-lock.json`, and `versions.json` together. Tag must match `manifest.json` version exactly (no leading `v`).

Releases are cut with the `$release` skill (`.agents/skills/release/SKILL.md`) --- it bumps all four version files, tags, pushes, waits for the build, and publishes. Don't bump or tag by hand.

## Coding conventions

- Keep `src/main.ts` minimal --- lifecycle and wiring only. All logic lives in sub-modules.
- Handwritten source files have a 500-line limit, excluding blank and comment-only lines; explicit path exceptions live in `scripts/source-size-exceptions.json`. Split by responsibility when it improves the code.
...
- User-facing strings go through `t()`, with the canonical key added to `src/i18n/en.ts`.
