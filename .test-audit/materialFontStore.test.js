// tests/materialFontStore.test.ts
...
// src/icons/materialFontStore.ts
var WOFF2_MAGIC = [119, 79, 70, 50];
var MAX_FONT_BYTES = 8 * 1024 * 1024;
var MaterialFontStore = class {
  ...
  path(family2) {
    const slug = family2.toLowerCase().replace(/[^a-z0-9]+/g, "-");
  ...
  async has(family2) {
    ...
    try {
      const exists = await this.app.vault.adapter.exists(this.path(family2));
  ...
  async write(family2, bytes) {
    ...
    try {
      ...
      if (this.destroyed) return;
      await adapter.writeBinary(this.path(family2), bytes);
    } catch (e) {
      if (!this.destroyed) console.warn(`[CalloutStudio] could not cache the "${family2}" webfont`, e);
    }
  }
  async remove(family2) {
    if (this.destroyed) return;
    try {
      await this.app.vault.adapter.remove(this.path(family2));
    } catch {
    }
  }
};
var store = null;
function setMaterialFontStore(app, manifest) {
  store?.destroy();
  store = new MaterialFontStore(app, manifest);
}
function clearMaterialFontStore() {
  store?.destroy();
  store = null;
}
function materialFontStore() {
  return store;
}
...
var validFont = Uint8Array.from([119, 79, 70, 50, 0, 0, 0, 0]).buffer;
function deferred() {
  let resolve;
...
function harness() {
  ...
  const manifest = { id: "callout-studio" };
  setMaterialFontStore(app, manifest);
  const store2 = materialFontStore();
  return { store: store2, app, manifest, adapter, files, calls };
}
describe("MaterialFontStore lifecycle", () => {
  ...
  it("clearing invalidates a captured reference and stops the read following exists", async () => {
    const h = harness(), exists = deferred();
    h.adapter.exists = () => {
      h.calls.push("exists");
  ...
  it("does not remove a corrupt read completed after replacement", async () => {
    const h = harness(), read = deferred(), entered = deferred();
    h.adapter.exists = () => Promise.resolve(true);
    h.adapter.readBinary = () => {
      h.calls.push("read");
      entered.resolve();
      return read.promise;
    };
  ...
  for (const stage of ["exists", "mkdir"]) {
