// tests/legacyStartupSnippet.test.ts
...
// src/manager/legacySnippetArchive.ts
async function archiveLegacySnippet(app, source, isActive = () => true) {
  if (!isActive()) return;
...
// src/manager/legacyStartupSnippet.ts
var LEGACY_SNIPPET_NAME = "callout-studio-do-not-delete";
async function removeLegacyStartupSnippet(app, isActive = () => true) {
  if (!isActive()) return;
  try {
    const adapter = app.vault.adapter;
    ...
    const wasEnabled = customCss?.enabledSnippets?.has?.(LEGACY_SNIPPET_NAME) ?? false;
    const exists = await adapter.exists(path);
    if (!isActive()) return;
    ...
    if (!isActive()) return;
    console.warn(
      `[CalloutStudio] failed to remove the legacy startup CSS snippet \u2014 delete "${LEGACY_SNIPPET_NAME}.css" from your snippets folder manually`,
      e
    );
  }
}
...
// tests/legacyStartupSnippet.test.ts
var SNIPPET = "callout-studio-do-not-delete";
var CONFIG_DIR = ".obsidian";
var SNIPPET_PATH = `${CONFIG_DIR}/snippets/${SNIPPET}.css`;
function deferred() {
  let resolve;
...
function fake(opts = {}) {
  const vault = {
    ...
    failRemove: false,
    failExists: false,
    content: /* @__PURE__ */ new Map([[SNIPPET_PATH, "/* personal changes */ body { color: red; }"]]),
  ...
  const customCss = opts.customCss === false ? void 0 : opts.customCss === "partial" ? (
    ...
    vault: {
      ...
      adapter: {
        async read(path) {
          ...
          if (text === void 0) throw new Error("ENOENT");
          return text;
        },
        async mkdir(path) {
          vault.present.add(path);
        },
        async write(path, text) {
          if (vault.failArchiveWrite) throw new Error("ENOSPC");
          vault.content.set(path, text);
          vault.present.add(path);
...
async function withQuietWarnings(body) {
  const seen = [];
...
describe("removeLegacyStartupSnippet \u2014 the vault that still has one", () => {
  ...
  for (const fault of ["failArchiveWrite", "corruptArchive", "changeSource"]) {
...
describe("removeLegacyStartupSnippet \u2014 the vault that never had one", () => {
  ...
  it("derives the path from the vault's own configDir", async () => {
    const f = fake();
    f.app.vault.configDir = ".obsidian-beta";
    await removeLegacyStartupSnippet(f.app);
...
describe("removeLegacyStartupSnippet \u2014 unload during cleanup", () => {
  ...
  for (const stage of ["first-read", "mkdir", "write", "final-read"]) {
    it(`preserves the active source if unloaded during ${stage}`, async () => {
      ...
      adapter.mkdir = async (path) => {
        await mkdir(path);
        if (stage === "mkdir") await pause();
      };
      adapter.write = async (path, text) => {
        await write(path, text);
        if (stage === "write") await pause();
      };
  ...
  it("does not reload snippets when an already-started removal completes after unload", async () => {
    const f = fake({ fileExists: true, snippetEnabled: true });
    const entered = deferred(), resume = deferred();
    const adapter = f.app.vault.adapter, remove2 = adapter.remove.bind(adapter);
    let active = true;
