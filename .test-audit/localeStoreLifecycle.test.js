// tests/localeStoreLifecycle.test.ts
...
function harness() {
  ...
  globals.__CS_REQUEST_URL__ = () => {
    calls.push("request");
    return Promise.resolve({ text });
  };
  const app = { vault: { adapter, configDir: ".obsidian" } };
  const store = new LocaleStore(app, { id: "callout-studio", version: "2.13.1" });
  let notifications = 0;
  store.onChange(() => {
    notifications++;
  });
...
describe("LocaleStore terminal lifecycle", () => {
  it("stops between exists and read, and refuses every new entry point after destruction", async () => {
    ...
    h.store.onChange(() => {
      throw new Error("listener retained after destroy");
    });
  ...
  it("suppresses a delayed cache failure notice after destruction", async () => {
    const h = harness(), entered = deferred(), write = deferred();
    h.adapter.write = () => {
      entered.resolve();
