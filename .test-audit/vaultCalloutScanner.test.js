// tests/vaultCalloutScanner.test.ts
import assert from "node:assert";
import { describe, it } from "node:test";

// src/utils/calloutId.ts
var splitCalloutMetadata = (raw) => {
...
var normalizeCalloutId = (raw) => splitCalloutMetadata(raw).id.replace(/\s+/g, " ").trim().toLowerCase();
var obsidianCalloutAttrId = (raw) => raw.trim().toLowerCase().replace(/\s+/g, "-");
var calloutIdentity = (raw) => obsidianCalloutAttrId(normalizeCalloutId(raw));
...
// src/utils/vaultRewrite.ts
async function forEachVaultFile(app, visit) {
...
async function rewriteVaultFiles(app, transform, requireComplete = false) {
  let files = 0;
  let total = 0;
  ...
  if (failed.length > 0) {
    if (!requireComplete) new Notice(t("notice.vaultRewritePartial", { count: failed.length }), 1e4);
...
// src/editor/inlineContent.ts
function blankInlineMath(line) {
...
function matchInlineContent(line, openIdx) {
...
// src/editor/calloutTokens.ts
var HEADING_CALLOUT_RE = /^(#{1,6})[ \t]+\[!([^\]\n\r]+)\][ \t]?(.*)$/;
var BLOCKQUOTE_CALLOUT_HEADER_RE = /^(\s*(?:>[ \t]?)+)\[!([^\]\n\r]+)\]/;
...
function scanLineForCalloutTokens(rawLine, options = {}) {
...
var FENCE_OPEN_RE = /^(?:\s*>\s*)*\s*(`{3,}|~{3,})/;
function createDocumentLineFilter() {
  ...
  return (line, index) => {
    ...
    if (inFrontmatter) {
      ...
      if (index > 0 && (t2 === "---" || t2 === "...")) inFrontmatter = false;
      return false;
    }
    const fence = line.match(FENCE_OPEN_RE);
    if (fenceMarker) {
      if (fence && fence[1] && fence[1][0] === fenceMarker[0] && fence[1].length >= fenceMarker.length && line.slice(fence[0].length).trim() === "") {
        fenceMarker = null;
      }
      return false;
    }
...
// src/editor/calloutWriter.ts
function splitFoldMark(afterBracket, role) {
...
// src/utils/calloutPlainText.ts
function applyEdits(source, from, to, edits) {
...
function rewriteLine(line, tokens, ids, name, outer) {
  const edits = [];
  for (const token of [...tokens].sort((a, b) => b.from - a.from)) {
    const children = token.content ? edits.filter((edit) => edit.from >= token.content.from + 1 && edit.to <= token.content.to - 1) : [];
    ...
    if (token.role === "inline") {
      const payload = token.content ? applyEdits(line, token.content.from + 1, token.content.to - 1, children).trim() : "";
...
function calloutsToPlainText(content, ids, displayName) {
  ...
  const converted = lines.map((line, index) => {
    ...
    const header = tokens[0]?.role === "regular" ? tokens[0] : void 0;
    const matchingHeader = header !== void 0 && ids.has(calloutIdentity(header.rawId));
...
// src/utils/vaultCalloutScanner.ts
function tokenBracketEnd(token) {
  return token.to;
}
function metadataSuffix(token) {
  return token.hasMetadata ? `|${token.metadata}` : "";
}
function splitFoldAndTitle(line, token) {
  return splitFoldMark(line.slice(token.to), token.role);
}
function rewriteTokensOnLine(line, tokens, replace) {
  let out = line;
  let count = 0;
  for (const token of [...tokens].sort((a, b) => b.from - a.from)) {
...
function rewriteCalloutLines(content, rewriteLine2) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const result = rewriteLine2(line, tokens);
...
async function convertCalloutsToPlainTextInVault(app, ids, displayName, requireComplete = false) {
  if (ids.length === 0) return { files: 0, blocks: 0 };
  const idSet = new Set(ids.map((id) => calloutIdentity(id)));
  const transform = (content) => calloutsToPlainText(content, idSet, displayName);
  const { files, count } = await rewriteVaultFiles(app, transform, requireComplete);
  return { files, blocks: count };
}
async function replaceCalloutIdsInVault(app, oldIds, newId, titleSwap, requireComplete = false) {
  ...
  const { count } = await rewriteVaultFiles(
    app,
    (content) => rewriteCalloutLines(
      content,
      (line, tokens) => rewriteTokensOnLine(line, tokens, (token) => {
...
async function replaceCalloutTitlesInVault(app, ids, oldTitle, newTitle, requireComplete = false) {
...
function fakeApp(files) {
  ...
  const read = (file) => Promise.resolve(store.get(file.path) ?? "");
  const app = {
    vault: {
      ...
      getAbstractFileByPath: (path) => handles.find((h) => h.path === path) ?? null,
      process: (file, fn) => {
...
async function rewrite(line, run) {
...
describe("replaceCalloutIdsInVault", () => {
  const swap = (line, titleSwap) => rewrite(
    line,
    (app) => replaceCalloutIdsInVault(app, ["old"], "new", titleSwap)
  );
  const named = (line) => swap(line, { from: "Old Name", to: "New Name" });
  ...
  it("reports one replacement per token, titled or not", async () => {
    const { count } = await rewrite(
      ...
      (app) => replaceCalloutIdsInVault(app, ["old"], "new", {
        from: "Old Name",
        to: "New Name"
      })
    );
  ...
  it("rewrites every id form it is given", async () => {
    const { line } = await rewrite(
      "> [!alias] Title",
      (app) => replaceCalloutIdsInVault(app, ["old", "alias"], "new")
    );
    assert.strictEqual(line, "> [!new] Title");
  });
});
describe("replaceCalloutTitlesInVault", () => {
  const rename = (line) => rewrite(
    line,
    (app) => replaceCalloutTitlesInVault(app, ["note"], "Old Title", "New Title")
  );
...
describe("convertCalloutsToPlainTextInVault", () => {
  const convert = (source) => rewrite(
    source,
    (app) => convertCalloutsToPlainTextInVault(app, ["zzz"], "Zzz").then(
      (r) => r.blocks
    )
  );
...
function hostileVault(files, opts = {}) {
  const store = new Map(Object.entries(files));
  const handles = Array.from(store.keys()).map((path) => ({ path }));
  const unreadable = new Set(opts.unreadable ?? []);
  const vanished = new Set(opts.vanished ?? []);
  const written = [];
  const read = (file) => unreadable.has(file.path) ? Promise.reject(new Error(`EACCES: ${file.path}`)) : Promise.resolve(store.get(file.path) ?? "");
  const app = {
    vault: {
      getMarkdownFiles: () => handles,
      read,
      cachedRead: read,
      getAbstractFileByPath: (path) => vanished.has(path) ? null : handles.find((h) => h.path === path) ?? null,
      process: (file, fn) => {
...
describe("a bulk rewrite survives a hostile vault", () => {
  ...
  it("keeps going when one note cannot be read", async () => {
    const notices = [];
    globalThis.__CS_NOTICES__ = notices;
  ...
  it("rewrites against the text on disk, not the text it probed", async () => {
    ...
    let landed = "";
    vault.cachedRead = (f) => probe(f).then((text) => {
      void vault.process({ path: "a.md" }, () => "> [!old] One\n\nTyped.");
      return text;
    });
    const inner = vault.process;
    vault.process = (f, fn) => inner(f, (data) => {
      landed = fn(data);
      return landed;
    });
